...
export default class PetType extends Component<StepProps, any> {
	...
	private static getIcon(item: any, selected: boolean) {
		return <Avatar.Icon style={{ backgroundColor: item.color }} icon={selected ? "check" : item.icon} size={40} />;
	}
	...
	renderType({ item }: { item: any }) {
		let selected =
			item.type === this.props.data.type ||
			(item.type === "other" && !this.flatTypes.includes(this.props.data.type) && this.props.data.type != "");
	...
	inputType(type: string) {
		if (type !== "") {
			...
			this.update(this.props.data.name, "");
		}
	}
	...
	render() {
		...
		return (
			<SafeAreaView style={styles.container}>
				...
				{!this.flatTypes.includes(this.props.data.type) && (
					<KeyboardAvoidingView>
						<TextInput
							label={"Type"}
